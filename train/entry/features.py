...
6. 学習用ラベル（将来の効率比 / Efficiency Ratio の連続値）を生成する
"""
...
def add_efficiency_ratio_label(df: pl.DataFrame, horizon: int = 240) -> pl.DataFrame:
    """
    将来のトレンド直進性（効率比 / Efficiency Ratio）の連続値(0.0 ~ 1.0)をラベルとして生成します。
    ...
    Returns:
        pl.DataFrame: 連続値の効率比ラベルが追加されたデータフレーム。
    """
    ...
    er = _safe_ratio(fwd_displacement, path_length)
    
    return df.with_columns(
        [
            er.cast(pl.Float32).alias(f"label_efficiency_{horizon}")
        ]
    )
